#!/usr/bin/env bash
# MPD ARM64 musl 平台构建脚本
set -e

echo "===================================="
echo "构建 MPD linux-arm64-musl"
echo "===================================="
...
# 设置 Alpine arm64 chroot 环境
CHROOT_DIR="/workspace/chroots/arm64-musl"
...
# 复制 musl libc
cp "${CHROOT_DIR}/lib/ld-musl-aarch64.so.1" "${TARGET_DIR}/lib/"
cp "${CHROOT_DIR}/lib/libc.musl-aarch64.so.1" "${TARGET_DIR}/lib/"
...
# 复制依赖库
ALL_LIBS=$(chroot "${CHROOT_DIR}" bash -c "ldd /workspace/stage/usr/bin/mpd /workspace/stage/usr/bin/mpc" | grep -o '/usr/lib/[^ ]*' | sort -u)
...
for lib in $ALL_LIBS; do
    ...
    if [ -L "${CHROOT_DIR}${lib}" ]; then
        ...
        cp "${real_lib_dir}/${real_path}" "${TARGET_DIR}/lib/${lib_name}"
    elif [ -f "${CHROOT_DIR}${lib}" ] && [ ! -f "${TARGET_DIR}/lib/${lib_name}" ]; then
        cp "${CHROOT_DIR}${lib}" "${TARGET_DIR}/lib/"
    fi
done
