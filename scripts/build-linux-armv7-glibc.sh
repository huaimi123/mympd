#!/usr/bin/env bash
# MPD ARMv7 glibc 平台构建脚本（Debian armv7 chroot）
...
# 设置 Debian armv7 chroot 环境
CHROOT_DIR="/workspace/chroots/armv7-glibc"
...
for lib in $ALL_LIBS; do
    ...
    cp -L "${CHROOT_DIR}${lib}" "${TARGET_DIR}${lib_dir}/${lib_name}" 2>/dev/null || true
done
